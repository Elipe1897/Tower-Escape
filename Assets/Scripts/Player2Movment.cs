...
{
    [SerializeField]                   //variabler som håller koll på vilka tangenter som styr Player2 -Lisa
    ...
    void GroundCheck()
    {
        ...
        if (colliders.Length > 0)
            if (isGrounded = true && (Input.GetKeyDown(KeyCode.UpArrow))) //om man nuddar en plattform och trycker ner upp piltangenten så hoppar Player2 -Lisa
            {
    ...
    void Update()
    {
        if (Input.GetKey(KeyCode.LeftArrow))     //om man trycker ner vänstra piltangänten åker Player2 vänster -Lisa
        {
